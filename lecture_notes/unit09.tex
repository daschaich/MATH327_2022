...
Once the universe was a few micro-seconds old, it cooled enough for this matter to transition into the protons and neutrons we are made out of today.
...
We now restore non-zero external magnetic field, $H > 0$, which allows us to gain a deeper appreciation of the magnetization.
\TODO{...}
...
\end{equation*}
From our earlier experience with the canonical ensemble, it now comes as no surprise that $\vev{M} = N\vev{m}$ can be related to a derivative of the Helmholtz free energy $F = -T\log Z$.
...
\end{equation}
defining an effective magnetic field $H_{\text{eff}}$ that depends on the mean spin.
