...
\renewcommand{\moddate}{Last modified 2 May 2022}
...
Repeating this for all terms in the product $\prod_{n = 1}^N$, the periodic boundary conditions introduce a (cyclic) trace, making the exact partition function simply
\begin{equation*}
...
$T$ is known as the \textbf{transfer matrix}, because it `transfers' information about the value of the spins from one link to the next.
We can see that rewriting the magnetic-field term in the energy just helped to make $T$ more symmetric.
If we are able to diagonalize
\begin{equation*}
  T = \begin{pmatrix} \la_+ & 0 \\ 0 & \la_- \end{pmatrix},
\end{equation*}
then will obtain the partition function $Z = \mbox{Tr}\left[T^N \right] = \la_+^N + \la_-^N$.
\TODO{...}
