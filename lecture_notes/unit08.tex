...
\renewcommand{\moddate}{Last modified 29 Mar.~2022}
...
If we invert that result, we obtain the so-called \textbf{Fermi energy} as a function of the average particle number density,
\begin{equation}
...
It is an interesting but lengthy exercise (discussed in Section~7.3 of Schroeder's \textit{Introduction to Thermal Physics}) to show that controlling the number of fermions in the gas requires the chemical potential to become negative as the temperature increases and we approach the classical limit.
...
\begin{align*}
  ...
  x \ll -1 \qquad \Lra \qquad & \frac{e^x}{\left(e^x + 1\right)^2} \approx \frac{e^x}{1} = \frac{1}{e^{-x}}.
\end{align*}
...
This is illustrated by the plot below, which shows the exponential suppression setting in when $|E - \mu|$ larger than a few times the temperature, and certainly for $|E - \mu| \gtrsim 5 / \be = 5T$. \\[-24 pt]
...
First, recalling $\mu > 0$ at low temperatures, for large \be (say $\be\mu \gtrsim 10$) we are free to extend the lower limit of the integral to obtain a more convenient domain of integration,
\begin{equation*}
...
Switching to work in terms of $T = 1 / \be$, the Sommerfeld expansion has given us a series of manageable integrals we can consider one by one:
\begin{equation*}
...
\end{mdframed}
Collecting the results, you should find
\begin{equation*}
  \vev{N}_{\text{f}} \approx \frac{2}{3} g_0 \mu^{3 / 2} + g_0 \frac{\pi^2 T^2}{12\mu^{1 / 2}}.
\end{equation*}
This becomes more interesting when we restore the explicit constant factors in $g_0$ from \eq{eq:number}, especially if we rearrange \eq{eq:Fermi_energy} to relate these to the Fermi energy $E_F$:
\begin{mdframed}
  $\displaystyle g_0 = V \frac{\sqrt{2m^3}}{\pi^2 \hbar^3}$ \\
  $\displaystyle E_F = \frac{\hbar^2}{2m}\left(\frac{3\pi^2 \vev{N}_{\text{f}}}{V}\right)^{2 / 3}$ \\[120 pt]
\end{mdframed}
...
Then
\begin{equation*}
  \frac{\mu}{E_F} \approx 1 - \frac{\pi^2 T^2}{12 \mu^2},
\end{equation*}
